...
import { playSound, playPageTurn } from './sonido.js';
import { rand, probabilidadBloqueo, sanitizar, idsVistos, lexicoActivo } from './utils.js';
...
export function animarFloresOrganicas() {
...
// PASTO ANIMADO //
export function animarPastoDenso() {
  const time = performance.now() * 0.001;
  pasto.forEach(g => {
...
export function crearVersoFlotante(texto, autor, label) {
  const canvas = document.createElement('canvas');
...
// Doble clic para compartir versos //
...
// ANIMACIÓN PRINCIPAL (loop de 60fps) //
let tiempo = 0;
